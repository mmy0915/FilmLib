...
mov_files_list = scan_files('F:\\FILM_LIB\\film_src\\', postfix='.mp4')
pro_des_path = 'F:\\FILM_LIB\\film_src_h264\\'
...
for f_name in mov_files_list:
    ...
    info = c.probe(f_name)
    try:
        mp4_vc_str = info.video.codec
    except:
        mp4_vc_str = ""

    i_count += 1
    print("%d / total: %d" % (i_count, len(mov_files_list)))
    print("mp4 video codex %s %s" %(mp4_vc_str, file_name))

    if mp4_vc_str.strip() != 'h264':
        ...
        param_str = " -threads 9 -codec:v h264 -codec:a aac -b:a 320k -y "
